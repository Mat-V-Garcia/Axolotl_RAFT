# AX - Axolotl Training for MagisAI
...
├── handler.py              # Serverless job handler
├── client.py               # Python client for submitting jobs
├── requirements.txt        # Client dependencies
├── example_config.yaml     # Reference Axolotl config
├── example_data_sharegpt.jsonl  # Sample chat training data
├── example_data_raft.jsonl      # Sample RAFT training data
└── README.md
```
...
AXOLOTL_ENDPOINT_ID=your_endpoint_id
```
...
result = client.train_from_file(
    data_path="example_data_sharegpt.jsonl",
    method="qlora"
)

# RAFT training
result = client.train_from_file(
    data_path="example_data_raft.jsonl",
    use_raft=True
)
```
...
```bash
# Standard training
python client.py --data example_data_sharegpt.jsonl --method qlora --epochs 3

# RAFT training
python client.py --data example_data_raft.jsonl --method qlora --raft

# Push to Hub
python client.py --data train.jsonl --hub-repo your-username/model-name
```
